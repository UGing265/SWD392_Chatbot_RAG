...
import { ClipboardList, CheckCircle2, ChevronLeft, Calendar, BookOpen, Clock, AlertCircle, Search } from "lucide-react";
...
export function TakeQuizView() {
  ...
  const historyMocks = [
    { id: "h1", quizTitle: "Bài kiểm tra giữa kỳ 1", subject: "Lập trình Web", score: 9.5, total: 10, date: "10/05/2026", time: "14:30" },
    { id: "h2", quizTitle: "Trắc nghiệm SQL cơ bản", subject: "Cơ sở dữ liệu", score: 8.0, total: 10, date: "08/05/2026", time: "09:15" },
    { id: "h3", quizTitle: "Quiz 1: HTML & CSS", subject: "Lập trình Web", score: 10.0, total: 10, date: "01/05/2026", time: "10:00" },
  ];
        ...
        <div className="max-w-4xl mx-auto">
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-zinc-50/80 border-b border-zinc-100">
                        <th className="p-5 text-xs font-bold text-zinc-500 uppercase tracking-wider">Bài kiểm tra</th>
                        <th className="p-5 text-xs font-bold text-zinc-500 uppercase tracking-wider">Môn học</th>
                        <th className="p-5 text-xs font-bold text-zinc-500 uppercase tracking-wider">Điểm số</th>
                        <th className="p-5 text-xs font-bold text-zinc-500 uppercase tracking-wider">Thời gian nộp</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-100">
                      {historyMocks.map((history) => (
                        <tr key={history.id} className="hover:bg-zinc-50/50 transition-colors">
                          ...
                          <td className="p-5">
                            <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[13px] font-bold ${history.score >= 8 ? 'bg-green-100 text-green-700' : history.score >= 5 ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'}`}>
          ...
          {!selectedTerm ? (
            ...
            <div className="grid gap-6 sm:grid-cols-2 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-100">
              {quizzes
                .filter(q => q.subject_name === selectedSubject.name && q.academic_term_name === selectedTerm.name && q.title.toLowerCase().includes(searchQuery.toLowerCase()))
                .map((quiz) => (
                <div
                  key={quiz.id}
                  ...
                  <div className="flex-1 mt-2">
                    ...
                    <div className="space-y-3 mb-6">
                      ...
                      <div className="flex items-center gap-2.5 text-[13px] font-medium text-zinc-500 px-1">
                        ...
                        Thời gian: <span className="font-bold text-zinc-700">{quiz.duration_minutes} phút</span>
                      </div>
                    </div>
                    <p className="text-[14px] text-zinc-500 line-clamp-2 leading-relaxed">{quiz.description}</p>
                  </div>
              ...
              {quizzes.filter(q => q.subject_name === selectedSubject.name && q.academic_term_name === selectedTerm.name).length === 0 && (
                <div className="col-span-full py-20 text-center bg-white rounded-[2rem] border border-zinc-200/60 shadow-sm animate-in fade-in zoom-in-95 duration-500">
                   ...
                   <p className="text-[15px] font-medium text-zinc-400 mt-2">Giảng viên chưa tải lên bài kiểm tra cho môn học này.</p>
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] bg-zinc-100 py-10 px-4 md:px-8">
      <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-300">
        ...
        <Button variant="ghost" onClick={handleBackToList} className="text-zinc-500 hover:text-teal-700 hover:bg-teal-50 pl-2 rounded-xl">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-zinc-100 text-center">
          ...
          <div className="flex items-center justify-center gap-6 text-sm text-zinc-500">
            <span className="flex items-center gap-2 bg-zinc-50 px-3 py-1.5 rounded-lg"><BookOpen className="h-4 w-4 text-teal-600" /> {selectedQuiz.subject_name}</span>
            <span className="flex items-center gap-2 bg-zinc-50 px-3 py-1.5 rounded-lg"><Clock className="h-4 w-4 text-orange-400" /> {selectedQuiz.duration_minutes} phút</span>
          </div>
        </div>
              ...
              >
                ...
                <div className="space-y-3 mt-6">
                  {q.options.map((opt) => {
                    ...
                    if (isSelected && !submitted) optionStyle = "border-teal-500 bg-teal-50 text-teal-800 shadow-sm";
                    if (submitted) {
                      if (isCorrectOption) optionStyle = "border-green-500 bg-green-100 text-green-800 font-medium shadow-sm";
                      else if (isSelected && !isCorrectOption) optionStyle = "border-red-400 bg-red-100 text-red-800";
                    ...
                    return (
                      ...
                      >
                        <div className={`h-5 w-5 rounded-full border-2 mr-4 flex items-center justify-center flex-shrink-0 ${
                          ...
                          {isSelected && !submitted && <div className="h-2.5 w-2.5 bg-teal-500 rounded-full" />}
                          {submitted && isCorrectOption && <CheckCircle2 className="h-3.5 w-3.5 text-white" />}
                        </div>
                ...
                {submitted && isWrong && (
                  <div className="mt-4 p-4 rounded-xl bg-orange-50 border border-orange-100 flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-orange-800">Câu trả lời chưa chính xác!</p>
                      <p className="text-sm text-orange-700 mt-1">
                        Đáp án đúng là: <span className="font-bold">{q.options.find(o => o.id === q.correctOptionId)?.text}</span>
                      </p>
                    </div>
                  </div>
                )}
